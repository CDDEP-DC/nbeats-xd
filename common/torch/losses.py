...
def mape_loss(forecast: t.Tensor, target: t.Tensor, mask: t.Tensor) -> t.float:
    ...
    x = (forecast - target) * weights
    return t.mean(x * t.sign(x))
    #return t.mean(t.abs((forecast - target) * weights))
    ## avoid using abs() for now because pytorch warns it's not implemented for mac gpu (lol?)
...
def smape_2_loss(forecast, target, mask) -> t.float:
    ...
    """
    d = forecast - target
    f_abs = forecast.data * t.sign(forecast.data)
    t_abs = target.data * t.sign(target.data)
    return 200 * t.mean(divide_no_nan(d * t.sign(d), f_abs + t_abs) * mask)
...
def mase_loss(insample: t.Tensor, freq: int,
    ...
    :return: Loss value
    """
    #x = insample[:, freq:] - insample[:, :-freq]
    #masep = t.mean(x * t.sign(x), dim=1)
    masep = t.mean(t.abs(insample[:, freq:] - insample[:, :-freq]), dim=1)
    masked_masep_inv = divide_no_nan(mask, masep[:, None])
    #d = target - forecast
    #return t.mean(divide_no_nan(d * t.sign(d), masep[:, None]) * mask)
